...
3. **Git/GitHub state:**
   ...
   - `python3 scripts/check_cadence.py --since 7d` — one row per cadence rule for every PR merged this week (review evidence, squash, CI, ledger touched, worktree reapable; cadence §4). This sweep is the **backstop** for the check: the working session and any orchestrator run it per PR, and a PR neither of them checked shows up here. A `review` fail means a merge went out without the `cadence-review:` comment — flag it, and note that a pass on that row is self-attested. A FAIL in its **Direct commits** section is a commit that reached main with no PR and outside §4's carve-outs — report it as STRANDED ("landed without review"), with the files it touched.
   ...
   - **The anchor itself (TD-033):** the main checkout — this repo's, and each roster repo's (`${DEV_CADENCE_REG_DIR:-${XDG_CONFIG_HOME:-~/.config}/dev-cadence}/repos.txt`) — belongs on the default branch. **Report — never act — "anchor parked on `<branch>` (idle Nh, no PR) — PR it, or `git checkout main`"** only when ALL hold, each run against `<root>`: `git -C <root> branch --show-current` is not the default branch; `(cd <root> && gh pr list --head <branch> --state open)` finds nothing; its last commit (`git -C <root> log -1 --format=%ct <branch>`) is older than **12 hours**; and no live session holds that checkout — `(cd <root> && python3 <root>/scripts/check_anchor.py)` (`files/scripts/` in dev-cadence itself); the script judges its *current directory*, so the `cd` is what points it at `<root>`. Any one failing means ordinary in-progress work — say nothing. Why it matters: memory is written into the main checkout by absolute path and never reaches main from a parked branch, and a worker that finds `main` checked out nowhere checks it out inside its worktree. This lives here, not in the SessionStart hook: it needs the network and a time window (the hook is local-only and budgeted).
   ...
   **When it says "not landed", ask one more question before reporting (TD-059).** The scoped test asks whether the branch's content is on main *now*; once main edits one of those files again, a branch that did land reads unlanded forever, and the sweep re-reports finished topics every run. So: did some first-parent commit on main since the merge-base make exactly the branch's change? `mine=$(git diff "$base" "$branch" -- "${files[@]}" | git patch-id --stable | cut -d' ' -f1)`, then for each `c` in `git rev-list --first-parent "$base..origin/main" -- "${files[@]}"`, compare with `git show --format= "$c" -- "${files[@]}" | git patch-id --stable`. A match is **landed, since edited** — list it under CLEARED, not STRANDED (`check_cadence.py`'s worktree row does the same) — **unless main's tip has undone it**: if `git diff --quiet "$base" origin/main -- "${files[@]}"` (those files are back to the merge-base), it landed and was reverted — report that under STRANDED/AWAITING THE USER (is the work still wanted?), never as cleared. When the merged PR's own squash commit is known, it alone decides: it must be on main (`git merge-base --is-ancestor <squash> origin/main`) and match the branch's files. No match (a squash that resolved conflicts, or main edited inside the hunk's context before the squash) stays "not landed": the safe direction. A coincidental identical change on main can still match — the cost of answering offline.
...
- **Transcript scan agent:** scan session tails under `~/.claude/projects/` — the main project dir (the munged project path, e.g. `-home-user-myrepo`) **and** worktree-path variants (`-*claude-worktrees-*`), since concurrent sessions run in worktrees. For the ~20 most recent sessions per dir: extract the last few assistant messages + any todo state from each JSONL tail (`tail -c 200000`, helper script in scratchpad — files are huge, never read whole). Flag sessions ending with pending todos, "held for your review", "next steps", or an unanswered blocking question to the user. Cross-check each flagged item against git log / the TD ledger before calling it stranded.
  **Also read the sessions that ran elsewhere (TD-28):** work on this repo routinely happens from a session running in *another* repo (a constellation, or a session that edited this repo by path), and its transcript lives under that repo's project dir. `python3 scripts/list_sessions.py --roster --mentioning . --days 30` (run from anywhere in this repo — `--mentioning` maps a worktree to its main checkout, the path other sessions use) lists every session on this machine's roster whose transcript names this repo's path; scan those tails the same way. A path in a transcript is a heuristic — these are **candidates** for you to judge, never a filter that decides for you (a session can name the path in passing; a session that reached the repo only through a symlink spelling nothing here resolves will be missed). Reading wider does not widen the stamp: `Swept-deep:` still goes on this repo's board only (TD-12's rule), and it now means this repo's sessions *and* the candidates from elsewhere were read. If `--roster` finds no roster, say so in the report — the sweep then covered this repo's own sessions only.
