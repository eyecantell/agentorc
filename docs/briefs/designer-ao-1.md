...
- **A defensible default he might care about** — you would choose, but he could reasonably choose otherwise (a label, a placement, which of two shapes): open the PR with your choice, then `ao msg --kind steer --default "<your choice>" --bound 12h --about TD-NNN person "<the choice, in one line, and the alternative>"`, and **go on to the next entry**. Merge at the bound if nothing came back, or when he says *go with it*; a reply saying otherwise is a change to the PR before the merge. A bound is never shorter than a night: he may be asleep.
