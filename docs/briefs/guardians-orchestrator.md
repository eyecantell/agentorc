...
- The tool is **Claude Code**, and the repo is a **dev-cadence consumer** — so the ledger, the
  ...
  they mean in this repo, and the lead's cadence-check duty carries over unchanged.
...
1. **The repos exist on the host the host agent runs on.** Until `~/dev/guardians` (or wherever Paul
...
2. **The devcontainer question is answered (design §10, raised 2026-09-13).** agentorc launches a
   ...
   the container has to reach the host agent's socket outside it. Three shapes are possible — run the
...
- **Never touch this repo's sessions**, and never act on a session that does not name you: two
  leads on one host is exactly the case membership exists for.
