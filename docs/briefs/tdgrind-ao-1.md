...
## Lane: free-pick TD grind
Scan docs/technical_debt.md for open entries suitable for autonomous completion — favor small/medium code fixes, test-coverage gaps, tooling, and doc corrections verifiable against code (TD-005/006/007/009/011/012/013/018/020/021 are small; TD-016/017 are medium and well specified). For each: verify current code state first (entries lag reality — grep before building). Loop: pick → fix + `pdm run test` + `pdm run lint` → PR → independent review by a **Sonnet** agent per docs/cadence.md §4 (an Agent with `model: sonnet` — never review on your own model; doc-only PRs get the same fact-check review) → address findings → squash-merge when SHIP → next. Resolved entries move to docs/technical_debt_archive.md with their summary row deleted (cadence §2).

**Avoid**: TD-022 and the `td022-fix` worktree (Paul's `aotest` session is live-testing PR #33 there); anything on docs/user_attention.md awaiting Paul's decision; TD-008 and the other design §10 open questions; TD-002/TD-003 (UI work Paul wants to see first); TD-004's ssh transport (multi-host is phase 2).
...
Check `date` between TDs. **Stop starting new work at 05:30 MDT (11:30 UTC) on 2026-09-10**, and stop earlier if Claude reports the 5-hour or weekly usage window is near its limit. Then: push every branch, strand nothing uncommitted, update the ledgers (items needing Paul → docs/user_attention.md with Due: dates), write a concise end-of-run summary as your final message (PRs merged, PRs open awaiting review, anything left), and exit (`/exit`).
