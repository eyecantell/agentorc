...
**Stop when your lane is done**, when Claude reports the 5-hour or weekly usage window near its limit (Paul is on the same subscription and his work comes first), or when your lead sends you the wrap-up prompt — whichever comes first. There is deliberately **no date in this brief**: a hard-coded stop time turns a repeatable `ao team start` into a session that exits the moment it starts, which is exactly what happened on 2026-09-13. Scheduling is TD-026's, not a brief's. Then: push every branch, strand nothing uncommitted, update the ledgers (items needing Paul → docs/user_attention.md with Due: dates), write a concise end-of-run summary as your final message (PRs merged, PRs open awaiting review, anything left), and exit (`/exit`).
