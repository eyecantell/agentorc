...
Two grinders pay only while their files do not meet. Your sibling holds the host agent: `src/sessionorc/agent.py`, `mail.py`, `models.py`, `identity.py`, and the ledger entries that live there (TD-079, TD-077, the mail half of TD-081). **Those entries are your sibling's whole, whatever file a part of them is in** — TD-079's page steps in `ui/` and TD-077's line in `cli.py` included — and **you do not pick any other entry whose fix is in those files**, even when the ledger shows it open and unclaimed; if a pick of yours turns out to need a change there, stop, say so to your sibling and your lead with `ao msg`, and take the next one.
