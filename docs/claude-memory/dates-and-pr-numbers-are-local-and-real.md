...
**A PR number is written after the PR is opened, never before.** I guessed `#307` in TD-077 (it was a sibling's open PR, not mine) and `#319` in TD-029 (someone else's board PR). The habit that works: commit the change, open the PR, then add the number in a second commit.
