...
| Money | Paperclip Labs, Inc.; the self-hosted product is free and MIT. Two ways to charge, neither priced publicly (no pricing page, 2026-09-24): **Paperclip Cloud**, a hosted multi-tenant instance on AWS behind a waitlist and *Sign up* on the site (`doc/SECRETS-AWS-PROVIDER.md`, `doc/DEPLOYMENT-MODES.md` *warm-pool identity*, the `v2026.824.0` onboarding seed from cloud signup), and **ClipHub**, a planned marketplace of team blueprints, agent blueprints, skills and governance templates at $0–$499 with a creator revenue share (`docs/specs/cliphub-plan.md`, superseded in shape by a *company package* format but not in intent). No funding announcement found. Against §4.5c: their hosted product runs the agents in their sandboxes — *we host your workspace* — and nothing like the relay (*bring your own machine*) exists |
