...
1. **Org** (home): a **card grid**, grouped by team when any live session carries a team badge
   ...
   **The card's anatomy (TD-095).** A **full card** — every card outside a live team's group, *No team*'s above all — is **six rows, the same six on every card, in the same
   ...
   **The Org, team-first (TD-176; Paul, 2026-09-26, four rounds on a design canvas).** Once a
   ...
   - **A member's compact card** (§4.5a *card: compact*): the name and, on a person's own session,
     ...
     keeps the amber ring and its foot leads with Focus; the answer is given in the facet, and the
     ring's `a` / `d` keys (§4.5a *Org: keys*) give it there.
...
11. **Repo** (`/repo/<name>`; designed 2026-09-25 with Paul, reshaped 2026-09-26 on the canvas —
   ...
   **The page opens with the team's three facets**, exactly as the team card draws them (screen
   ...
   servicing team. **A registered repo no team services** has a page too, reached by its URL and
   from `ao repo`, never linked from the Org: the Repo facet alone at the top (its numbers hold
   without a team), *TDs in motion* reading *no team services this repo*, no Answer needed /
   Doing facet, and the lists below with the Doing section empty. Where the numbers come from is §4.4 *Repo facts*; the CLI's read is `ao repo`
   (§4.7).
...
| card | **compact** (a team member's card; built by TD-176 slice 3 — when a team comes alive or winds down, every member's card is redrawn in its new shape) | the shape every member of a live team has (§4.5 screen 1): the name, the person glyph on the person's own session, the state pill (word and colour, no glyph), one line of its own — the role and its claim with the PR, or its ending, or the seat's *last came*, or the manager's round — and the foot with the full card's rule (the next act outlined, the rest plain, *more* at the right; a `needs-you` member's foot leads with Focus, the answer being the facet's). Every control of the full card's *more* is here too. The full card stays on *No team* and wherever a card is drawn outside a team |
...
**When it is read: the sentence the sender sees (TD-158; designed 2026-09-25, built by TD-168).**
