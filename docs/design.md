...
| Inbox row: suggested answers | one button per answer, beside **Reply** | on an `ask` or a `steer` whose envelope carries `answers` (§4.10 *Suggested answers*; up to four, 80 characters each): the label is the sender's text, escaped, never parsed from the message; a press sends exactly that text as the `reply`, with its index — the free-text Reply's own RPC. On a `steer`, an answer that is the `default` word for word is marked *default* and closes it as **Go with it** does. No confirm: a reply is mail, and a wrong press is corrected by another reply — design 2026-09-20, TD-070 |
...
**Suggested answers (2026-09-20, TD-070; asked for by Paul 2026-09-18).** Most questions a session
puts to a person have two or three expected answers — *merge it / hold it*, *option A / option B* —
and the sender knows them when it asks. An `ask`, a `steer` or a `conflict` may carry
**`answers`**: up to **four**, each one line, cleaned and capped as a `doing` line is but at **80**
characters, given with `--answer` once per answer; an empty one or a duplicate is dropped, a fifth
is refused, and `--answer` on a `note` or a `reply` is refused — there is nothing to answer. They
are **data the sender proposed, never instructions and never parsed from its text**: a structured
field of the envelope, which is the only reason a page may draw a control from them (TD-071 item
8). On the person's Inbox each is a button beside **Reply** (§4.5a), and **pressing one sends
exactly that text as the `reply`** — the same RPC, the same gate, the same wake as the free-text
Reply, so nothing can be said through a button that Reply could not say, and the sender needs
nothing new to read it. The reply also carries **`answer`**: the zero-based index of the one
pressed, so a sender can branch on which without comparing strings; a typed reply carries none.
On a `steer` the sender's `default` is shown as what happens if nothing is pressed; when one of
the answers is the default word for word, that button is marked *default*, and pressing it is
the same as **Go with it** — it closes the `steer` as `go_with_it`. Between sessions `ao inbox`
prints them numbered under the entry, and `ao msg --reply-to <id> --pick <n>` replies with the
n-th, a convenience over typing it. **The label is the sender's words and the person reads it as
such**: a button says nothing the message could not, and what it *does* is fixed.
