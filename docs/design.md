...
exactly the stranded work the check exists for; **and *pushed* is measured against the branch's own
remote branch, `origin/<branch>`, when one exists, and against the upstream only when none does**
(2026-09-20, TD-080): *ahead of the upstream* is *unmerged* when the upstream is `origin/main`, which
is a different question from *exists only on this machine*, and the check — like the card's
*n unpushed* flag and the Inbox row built from it — is about the second. A launch branch that tracks
`origin/main` and is pushed to `origin/<branch>` at wrap-up read as *308 unpushed* for ever, and a
row that is always there teaches a person to ignore the row. The host agent reads the count with
`git rev-list --count origin/<branch>..HEAD` against the remote-tracking ref it already has — it
never fetches — and reports which ref it measured in `git.pushed_against`, so the page can say
*3 unpushed · vs origin/td-080*), `gh pr view --json state` merged (when the branch
...
| Focus (exited / closed) | **Resume** / **Resume with changes…** / **New session here** / **Forget** | the exited banner. **Resume** (2026-09-20, Paul, TD-081: *a resume option that requires no input from me*) is one press and no form: a `create` on the record's host with the record's own `name`, `dir`, `adapter`, `profile`, `role`, `team`, `project`, `lane`, `controllers` and `unattended`, and `resume` = the tool's session id — so the name check answers `supersede`, the new session takes the bare name, the old record is closed in place and its mail moves (§4.10 *Resume carries mail forward*). **Not carried:** `run_until` and `wrapup_prompt` (a deadline that has passed is not a deadline; the create's ordinary default applies, §6), `prompt` (a resumed conversation needs no first prompt), `capabilities` (derived again from the role, never copied from a record). It is a person's act from the page — no `caller`, no attenuation (§4.8 create rule) — and has no CLI form beyond the `ao new --resume` that exists. **When it cannot be silent it is not a guess:** no tool session id on the record, a name a *live* record holds, a directory that is gone, a profile that no longer exists, or a host that is not connected — the press lands on the form, filled in, with the reason on it. **Resume with changes…** is that same filled-in form, asked for: New session with every field above prefilled (before 2026-09-20 the link carried the directory and the adapter and **no name**, so a person typed one and got a second record beside the one they were resuming). **New session here** prefills the directory only; Forget removes the record (the pane and its run log stay readable until then); CLI: `ao forget <id>` (the `remove` RPC; refuses a live record) |
