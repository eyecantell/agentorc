...
| Inbox section: **Waiting on them** | **Dismiss** | answered questions that owe an outcome (§4.10 *Outcomes*) and whose asker is still live: the question, the answer given, how long ago, the asker's name and `doing` line. Never counted — it waits on a session, not on the person. **Dismiss** says *I do not need to hear back* and ends the debt. When the asker has **exited without reporting**, the row is under *Needs you* instead, counted, with **Open** and **Dismiss** — design 2026-09-20, TD-079 |
| Inbox: the FYI count, **Dismiss all** | the top bar's second number; one button | *Inbox 1 · 5*: the second number is FYI's entries, never added to the first (§4.10 *The Inbox is a queue*). The FYI section opens itself when its count is higher than this browser last saw. **Dismiss all** confirms once and dismisses what is listed — the trail and closed questions included, never an open question — design 2026-09-20, TD-079 |
...
1. **Nothing leaves without an answer.** Reading, opening, focusing, following a row's link: none
   ...
   Acknowledge, Allow / Deny — or by **resolving**, below. What retention prunes is a *trail*, never
   an item: an entry that was already answered or already resolved.
2. **What resolves itself leaves a trail.** A state row is a view of a record, and its need can go
   ...
   *pushed*, *forgotten*, *the limit reset*, *acknowledged by you* (an identity alarm, §4.8a), and
   plain *resolved* when it cannot tell. FYI lists the trail; **Dismiss** removes an entry early.
   A state the page never showed (it came and went between two polls) leaves a trail all the
   same: the trail is the home's, not the browser's.
3. **FYI is counted, quietly, and cannot hide mail.** Beside the main number the top bar shows a
   ...
   the trail, and closed questions inside their retention. It is never added to the first: the
   ...
   below) — and **Dismiss all** (one confirm) clears what is listed. Found on the day this was
...
- **`ao msg person --outcome done|blocked|dropped "<one line>" --about <ask id>`** — a `note` whose
  envelope carries `outcome` and whose `root` is the question's thread. The home checks that the
  named entry is the caller's own question to the person and that it owes an outcome (else refused,
  in words), then stamps **`outcome: {state, text, at, note}`** on the person's copy of the
  question. One line, cleaned and capped as a `doing` line is; *done* names its reference (a PR, a
  commit, a TD) in the line, as a report does (§4.8).
- **a new `ask` or `steer` on the thread** — `--thread <ask id>` — when more direction is needed:
  it lands in *Needs you* (or *Steering*) **with the thread above it** — the first question, the
  answer given — and settles the first as `outcome: asked_again`; the new one, once answered, owes
  its own.
A question that owes an outcome is **not pruned** while it owes one, and it **holds its sender's
slot** in the person-inbox depths exactly as an open question does — the pressure to report is the
same pressure as the pressure not to over-ask. It stops owing when the outcome lands; when the
person **Dismisses** it (*I do not need to hear back*); or when the asker's record is **closed or
forgotten**, which closes the debt as `outcome: asker_gone` and says so. Where it shows:
**Waiting on them** — a fourth section of the page, under *Steering*, never counted: answered
questions that owe an outcome and whose asker is still live, each with the answer given and its
age. **Needs you** — counted — when the asker **exited without reporting one**: only a person (or
the asker's manager, whose brief gains it) can find out what happened, and the row offers **Open**
(the session's details, Resume) and **Dismiss**. **FYI** — the outcome itself, shown under the
question it closes: *you said "merge it" → done: merged as #261*. A lapsed `steer` owes nothing —
nobody answered — and a declined question owes nothing either.
