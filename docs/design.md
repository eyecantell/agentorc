...
- **A lead that was busy still sees it.** Mid-turn a lead is not blocked on anything, and a lead that misses the one event it existed for is worse than a poll. `subscribe` opens with a snapshot of every record, so the first thing `ao wait` does is compare that snapshot against what this caller last *saw* — a cursor it keeps per caller — and return at once if anything moved while it was away. Only then does it listen. A first wait records where it is and wakes on nothing, so no lead's first call returns its whole fleet.
- **Nothing is sent into the lead's pane.** The obvious reading — a worker messaging its lead — is the wrong one and is recorded here so it is not re-proposed: an acting RPC is gated on the *target's* `controllers`, so a worker acting on its lead would need the edge the design deliberately leaves empty (§4.9), and `send` is keys into a pane, which for a lead mid-turn is an interruption rather than a message. The worker already declares what matters through `ao progress` and `ao finding`; the agent, the one process that sees every record, is what turns a declaration into a wake.
