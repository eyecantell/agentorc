...
**A person's Start on a concluded team** (TD-099: `teamrun.concluded` and the page are built; the host agent's close-then-start, step (1), is not). A team whose every live session is `idle` and
