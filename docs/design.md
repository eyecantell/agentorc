...
| team header | **the on-demand member's button** — labelled by the role's `label` (*Designer* for the `designer` role; the label is the role's, never a word of the page's) | one button per member the definition carries with `start: on-demand` (§4.9 *An on-demand interactive member*, TD-120 step 3), on the team card's header beside its other controls, on a live, stopped or concluded team alike, since the person's seat does not wait for a run. With no live session under the definition's name it **starts one** — the same `create` as `ao team start` would make for that member: the name, worktree, badge, brief with `{manager}` and `{techlead}` filled (`none` where nobody holds them), profile, `unattended: false`, `start: on-demand`, no `supervised` — and opens its Focus; with one live it **raises its Focus** (the card's **Focus** link's behaviour: the popped-out window where this browser holds one, TD-046, else the tab). It never stops anything: the session's Close and Forget are on its own card and Focus. Refused, in the toast, when an `exited` or `closed` record holds the name with uncommitted or unpushed work (the wrap-up close's check), or the name is suspended (§4.8a), or the checkout is missing — the same refusals `ao team start` gives, one member at a time. A person's press, so no attenuation (§4.8) |
...
repo or `org.yml` defines with its own brief, not a preset, seated on a team as an on-demand
interactive member (§4.9) that the team card's button starts; `director` keeps
...
while the person is elsewhere; an interactive member is the exception and is said so), and
**`start`** (default `with-team`; **`on-demand`**, below). A member
...
**An on-demand interactive member (TD-120 step 3).** A team may seat a session that is the
person's own — the designer (§4.8 *Role names*), or any interactive role a repo defines — with
`{role: designer, name: designer-ao-1, unattended: false, start: on-demand, brief: …}`. `start:
on-demand` is legal only with `unattended: false` (refused by name otherwise: a session nobody
starts and nothing supervises is not a worker), `count` above one is refused on it, and it
changes three things and no more. **`ao team start` does not create it** — the person does,
from the team card's button (§4.5a **team header: the on-demand member's button**) or by hand
under the definition's name; when a live session holds the name the start is not refused for
it, since the name is not one the start will take. **Nothing counts it toward the team's
ending**: §4.9a's *every member is finished* and the page's *concluded* test skip it, as they
skip a seat, and `ao team stop` neither wraps it up nor closes it — an interactive session is
out of every controller's reach (§9 invariant 5), and the stop says so in one line rather than
naming it as left open. **Nothing acts on it**: the tick's four rules key on `supervised`, which
an interactive session never carries (§6 *Keeping a team running*), so it is never restarted,
nudged, paused or filled; its Close is its own, on Focus, so there is no toggle. What it gets
from the team is the rest: the badge (its card sits in the team's group, marked ***interactive***
as any of the person's sessions is, counted as live), its brief with `{manager}` and `{techlead}`
filled at its own start, the mail edges a badge gives (§4.10 *sideways*), and its worktree under
the team's naming. `start` is the definition's word and, once started, a field on the record
(`start: on-demand`, home-owned, written at create by whichever client starts it under the
definition's name, read by the concluded test and by `ao team stop`; absent on every other
record — §9 invariant 9: a badge, never a role, keys it). A seat (§4.9b) is the other thing a
team holds that is never finished; the difference is who starts it: a seat comes when a trigger
fires, an on-demand member when a person presses. Rejected: making the designer a `seats:`
entry — a seat is filled by its manager, and a person's session is started by no session.
...
| `manager` | every member is **finished** (below): none is working or waiting on something, and none exited without saying why — an on-demand interactive member (§4.9) and a seat (§4.9b) are not among *every member* |
...
has declared — `out_of_work` or `restart_wanted` — with its seats idle or gone, and its
on-demand interactive members (§4.9) not looked at, whatever their state (§4.5a *team
