...
- Board write-back (the RPC is built, TD-069 step 3; the Inbox's board rows press it since #472).
  **Snooze** (edit the `Due:` date, or add one) and **Done** (`- [ ]` → `- [x]`) on a dev-cadence
  ...
  checkout never sits dirty and the history is auditable. One **add** (TD-069 step 4; the RPC built,
  TD-140 slice 1 — the Inbox's button not yet): **Put on the board** on an FYI row (§4.5a) writes one new line at the top of the
