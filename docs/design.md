...
**A person's Start on a settled team (2026-09-22, TD-099).** A team whose every live session is
`idle` and has declared — `out_of_work` or `restart_wanted` — with its seats idle or gone (§4.5a
*team groups* defines **settled**, the team's word, beside *finished*, a member's: a member that
...
settled session — under the same check a wrap-up's close runs, so one with uncommitted or unpushed
work is not closed and the start is refused naming it — and supersedes the record under its name
before any create; a live session that is not settled is still refused by name, as today. A
...
*settled · restart wanted* and a person's Start after the reset is one press; once the gate pauses
