...
| card | **under `<controller>`** chip | the session's `controllers` when it has any — the controlling session's name, click to focus it; several are listed. **Not drawn inside a team's own group when the only controller is that group's manager** (design 2026-09-21, TD-095, not built): the group says it. Nothing is shown when the list is empty, which is the common case for a person's own session — landed 2026-09-13, TD-036 step 3 |
...
- `doing` (2026-09-19, TD-074; decided by Paul): **one line, the session's own word for what it is
  ...
  round; **a team card's header shows its manager's line** (as built 2026-09-19; **reversed by Paul 2026-09-21**, TD-095, not built yet: the line is on the manager's card, the first in the group, and the header stops repeating it — §4.5 *The card's anatomy*), which is the manager reporting on the team
...
state), projects, and the needs-you count across its members (from TD-095 on — design 2026-09-21, not built — the header carries the team, the host / repo its sessions share, the counts by state, its marks and its controls, and **not** its manager's name, state or line, which are on the manager's card: §4.5 *The card's anatomy*; until then it is drawn as described here) — the manager's card first, its
