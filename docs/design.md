...
`conflict` may carry **`answers`**: up to **four**, given with `--answer` once per answer. Each is
one line, capped at **80** characters, and cleaned **more strictly than displayed text is**: the
...
what it is not. One that cleans to nothing, or that repeats an earlier one exactly (compared after
