...
2. **Focus**: embedded terminal (full conversation; **on an `interactive` session the keyboard
   ...
   **two lines**, an identity line and an acts line — the split TD-085 named as the other answer
   ...
   side panel.
   1. **The identity line** — what this session is: ← Org, `host / repo / name`, the tool's
      title, the state pill with the pending's Allow / Deny, its *why?* box and its text while
      the session needs the person, then the marks — ***ready to close ✓***, the card's caption
      in the card's words, the moment the checklist passes on an `idle` or `exited` session;
      *suspended*; the *stops* note, drawn only when a stop time is set and still the control
      that edits it; *out of work*; *restart wanted*; *paused · usage* — and the *doing* line
      last, clamped. Nothing on this line is a button but the answer to a permission and the
      stops note.
   2. **The acts line** — what a person may do: the **next act first, outlined**, by state —
      **Close session** when the checklist passes (the same kill and reap as the card's, §4.2),
      **Take over** on an unattended session, else nothing, since a working or idle session's
      next act is the composer's Send; then **Message**, **Wrap up**, **Open shell here** and the
      editor button, plain; and at the right **more ▾** — Pop out, Copy and Paste (each naming
      its keys), Hand back / Switch to unattended when the toggle is not the next act, Copy tmux
      command, and **Kill**, last and red, where the card keeps it. Kill is never the only stop
      in sight: a session the checklist passes offers Close where Kill stood, and Kill is one
      menu away for the case where the worktree must be kept.
   ...
   4. **The side panel** — every card a fold, remembered per browser as a team's fold is, in the
      order a person reads them: **Git** (open); **Reports** and **Inbox** (open, drawn only when
      non-empty, the count in the heading); **Members** on a manager (open); **Ready to close**
      (open — the checklist, with a small Close beside its heading so the press is beside the
      reason, the header's outlined one being the same act); and **Session**, last and
...
| Focus header | **Close session** | the acts line's next act, outlined, when Ready to close passes on an `idle` or `exited` session (§4.5 *The Focus screen's anatomy*, TD-156): the same kill + reap → `closed` as the card's. The identity line carries ***ready to close ✓*** in the card's words at the same moment. The side panel's Ready to close keeps a small Close beside its heading, the press beside the reason |
| Focus header | **more ▾** | Pop out · Copy · Paste (each naming its keys) · Hand back / Switch to unattended (when the mode toggle is not the next act) · Copy tmux command · **Kill** — what is rarely pressed, behind one menu as on the card (TD-156) |
