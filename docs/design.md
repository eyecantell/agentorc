...
session pages, the CLI, teams, mail between sessions, the home/node split with a container node, and
identity on one host are built. §7 lists what each phase still lacks;
...
§3), byte-identical to dev-cadence's seed (a parity pair; `CADENCE_HOOK_LINE`), and uses it when
...
- **The doorbell is the forwarded `wait`.** A `wait` from a node blocks at the home under
  ...
  nothing. Not built: the ring across the link.
...
| team header | **PRs waiting** count | Not built (TD-093, §4.9b *The reader*): *`n` PRs waiting · oldest `<age>`* from the seat's `prs_waiting: {n, oldest}` — a count and a time, never the entries; display only, not pressable (the entries are `ao inbox <seat>`'s). Absent without a seat, or when no session of the team carries `review`. And on the person's Inbox, an `ask` that carries `pr` (a `reader: person` repo) draws `#<n>` as a link to the PR beside its text — the number is a structured field, the text stays text |
...
hand-started session the project's reach block. Not built: a nested `{team: …}` member is
...
[--pick <n>] [--outcome done|blocked|dropped --for <ask id>] [--thread <ask id>]` addresses a
...
`--answer` and `--pick` are TD-070. `ao msg person "…"` addresses the org's person inbox,
...
**A person's Start on a concluded team** (TD-099). A team whose every live session is `idle` and
...
- **The reader: a PR waits for the techlead (TD-093).** **It is a setting of the role** — some
  ...
  **`prs_waiting: {n, oldest}`** (a number and a time, never their text, §4.10), which the team's
  ...
  reviewer found — it is the high-trust read the rule exists for. Not built: the reader — `review:`,
  `pr`, `prs_waiting` (TD-093).
...
to a person's message, naming no addressee, lands in the person inbox (TD-052 step 2). An unread
message changes no state; it is read when the person looks.
...
1. **Nothing leaves without an answer.** Reading, opening, focusing, following a row's link: none
   ...
   name is not a control: §4.8a *An alarm's answers*) — or by **resolving**, below. A question that
...
2. **What resolves itself leaves a trail.** A state row is a view of a record, and its need can go
   ...
   (an identity alarm, §4.8a; beside *logged by you → `<controller>`*, not built; a suspension ends
...
- Hosts: `~/.agentorc/hosts.yml` on every host — the UI host's copy lists the hosts; each host
  ...
  are the node→home link of §4.4a. **`home:`** names the host whose agent holds the org's graph
...
- **Usage gate** (per profile; designed, being built — TD-100): pause every unattended session on
  ...
  **A pause is a send, not a kill** (the PAUSE flag of this section's last bullet): the host agent
  ...
  `gated: {profile, label, pct, line, since, next, sent_at}` — `next` being when the line next
  moves or the window resets, whichever is sooner, and `sent_at` when the pause prompt **landed**,
