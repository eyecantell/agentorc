...
| team header | **PRs waiting** count | Not drawn yet; its field is built (TD-093, §4.9b *The reader*): *`n` PRs waiting · oldest `<age>`* from the seat's `prs_waiting: {n, oldest}` — a count and a time, never the entries; display only, not pressable (the entries are `ao inbox <seat>`'s), and never a board line — a held PR's wait is here and in the reader's inbox, nowhere else (§4.9a, TD-125). Absent without a seat, or when no session of the team carries `review`. And on the person's Inbox, an `ask` that carries `pr` (a `reader: person` repo) draws `#<n>` as a link to the PR beside its text — the number is a structured field, the text stays text |
