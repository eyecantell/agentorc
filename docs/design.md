...
in it. A team that is live and **finished** — every live session declared, the rest gone — is drawn as a stopped one, with **Start** and no wind-down (2026-09-22, TD-099; §4.5a *team groups*). The order down the page is what needs looking at first: the teams with something live, the
...
**A person's Start on a finished team (2026-09-22, TD-099).** A team whose every live session has
declared — `out_of_work` or `restart_wanted`, the seats excepted — is finished with sessions still
there, since a worker's `/exit` does not leave and a manager on an older brief may not close itself.
Its card offers **Start** and no wind-down (§4.5a *team groups*), and `ao team start` on it, from
this date, closes each declared live session — the close a wrap-up would have made, safe for the
same reason: its work is pushed, or the board already says what is not — and supersedes the record
under its name before any create; a live session that has not declared is still refused by name,
as today. A suspended record is untouched: it refuses every create (§4.8a). **Until the §6 gate is
built (TD-100)**, a manager that stops the team for the usage window declares it — `ao progress
restart --why "usage window, resets <time>"`, after its members have settled — so the team reads
*finished · restart wanted* and a person's Start after the reset is one press; once the gate pauses
