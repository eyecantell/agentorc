...
| Org | **Teams** strip: **Start / Stop** per definition | every team in `org.yml` and the repos' `.agentorc.yml`, its source and live count; Start runs the same sequence as `ao team start` (all checks before any create), Stop the same as `ao team stop` (wrap-up members, then the lead; **Stop now** kills). Collapsed to a count when nothing is defined (§4.9) — landed 2026-09-13; the wrap-up wait runs behind the response, so the page reports what was sent and the state deltas show the rest |
...
says what was sent and names the lead that follows, and the state deltas show the rest. New
