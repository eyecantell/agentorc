...
| **Paperclip** (MIT, https://paperclip.ing, [ADR](decisions/2026-09-24-paperclip.md)) | an "AI company" control plane: org chart, goals, tickets; each agent a finite heartbeat run per wake, resumed by session id, over twelve runtimes; Postgres, one server, sandboxed execution workspaces; dollar budgets with hard stops | continuation reconstructed from records (and its looping postmortem, for §6's ceilings); wake admission as a pure proceed/coalesce/defer function; budget policy as one generic row; decision queues with recorded precedent (§4.9b) | no live session to take over; permission prompts skipped by default; work lives in its database, not the repo; cents, not usage windows |
