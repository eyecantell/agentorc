...
- Board write-back (the RPC is built, TD-069 step 3; the Inbox's board rows press it). **Snooze** (edit the
  ...
  never sits dirty and the history is auditable. One **add** (designed, TD-069 step 4; not built —
  TD-140): **Put on the board** on an FYI row (§4.5a) writes one new line at the top of the open
  items, in the board's own format, committed as `agentorc: board <item head> (from <entry id>)`;
  the only line this system ever adds to a board, and only at a person's press — no session and
  no policy adds one (a session writes its own lines by PR, cadence §4). One **reply** (designed, TD-126;
  not built — TD-142): **Reply** on a board row appends one indented line under the item, `  — Paul,
  <date>: <text>`, committed as `agentorc: reply on <item head> (by the person)`; the item's line is not
  edited, and the row's mail half (§4.5a) runs only after the commit lands. The call is **`board_edit {board, line, text,
...
| Inbox board row | **Reply** | designed (TD-126; decided by Paul 2026-09-24 — *a board reply always goes on the board, and is mailed as well only when a live session still holds the context*; not built — TD-142): a compose box on a due board row. Confirm does two things and says which it did. **Always, the file**: the person's words are appended directly under the item as one indented line, `  — Paul, <date>: <text>`, through the write-back's third action, **reply** (§4.4; committed as `agentorc: reply on <item head> (by the person)`), the item's own line untouched, so it stays due and counted — **a reply is not Done**; the session that carries the instruction out closes the line as any is closed. The next session started in that repo reads the reply from the SessionStart hook once dev-cadence's reader gives an item's indented lines with it (TD-142's dev-cadence half; until then the hook prints the item line alone). **When a lease holds the line's reference, the mail too**: the reader gives the item's `Context:` references (`context`, a structured field — never parsed here, §4.5 screen 6), and every live record whose `progress` holds a `claimed` entry on one of them with no later `done` or `dropped` (the lease, §4.8 — at most one per reference) receives a **`note` from the person**, `about` that reference, its text the item's head then the person's words; a person's message wakes and is never counted (§4.10). It is a `note` and not a `reply` because it answers no entry: the board line was never mail. A sender that has moved on, or exited, is not written to (Paul: *will it have moved on and the reply is a distraction, or will it already have the context?* — the lease answers that). **The row says what it did**, from the RPC's structured reply: *written under the line*, or *written under the line · sent to grinder-ao-2 (holds TD-122)*, the name a link to the session. Refused as Snooze and Done are when the checkout cannot take the commit; nothing is mailed when the file write fails |
...
| Focus composer | placeholder | display only: *first what you want, then why* — the person is asked the shape §4.10 asks of a session's message to them (TD-127; TD-139) |
...
designer's brief — and the person is asked the same thing the other way: the Focus composer's
placeholder reads *first what you want, then why*. **The home warns, never refuses.** `ao msg` to
...
link is the one pressable thing text can make**, and §4.5a's rule (*nothing on the page is built
