...
- **Outcomes reported** (§4.10 *Outcomes*, TD-079; built): nothing the person answered is still
  owed an outcome.
- **Mail read** (TD-072; designed 2026-09-24, not built — TD-136): no unread entry in the
...
- Board write-back (the RPC is built, TD-069 step 3; the Inbox's board rows press it). **Snooze** (edit the
  ...
  TD-135): **Put on the board** on an FYI row (§4.5a) writes one new line at the top of the open
  ...
  not built — TD-137): **Reply** on a board row appends one indented line under the item, `  — Paul,
...
| Inbox board row | **Reply** | designed (TD-126; decided by Paul 2026-09-24 — *a board reply always goes on the board, and is mailed as well only when a live session still holds the context*; not built — TD-137): a compose box on a due board row. Confirm does two things and says which it did. **Always, the file**: the person's words are appended directly under the item as one indented line, `  — Paul, <date>: <text>`, through the write-back's third action, **reply** (§4.4; committed as `agentorc: reply on <item head> (by the person)`), the item's own line untouched, so it stays due and counted — **a reply is not Done**; the session that carries the instruction out closes the line as any is closed. The next session started in that repo reads the reply from the SessionStart hook once dev-cadence's reader gives an item's indented lines with it (TD-137's dev-cadence half; until then the hook prints the item line alone). **When a lease holds the line's reference, the mail too**: the reader gives the item's `Context:` references (`context`, a structured field — never parsed here, §4.5 screen 6), and every live record whose `progress` holds a `claimed` entry on one of them with no later `done` or `dropped` (the lease, §4.8 — at most one per reference) receives a **`note` from the person**, `about` that reference, its text the item's head then the person's words; a person's message wakes and is never counted (§4.10). It is a `note` and not a `reply` because it answers no entry: the board line was never mail. A sender that has moved on, or exited, is not written to (Paul: *will it have moved on and the reply is a distraction, or will it already have the context?* — the lease answers that). **The row says what it did**, from the RPC's structured reply: *written under the line*, or *written under the line · sent to grinder-ao-2 (holds TD-122)*, the name a link to the session. Refused as Snooze and Done are when the checkout cannot take the commit; nothing is mailed when the file write fails |
...
| card `more ▾`, Focus header | **Message** | opens a composer that sends a `note` or `ask` from the person into this session's inbox (host agent RPC, ungated for a person; `from` is the person). Mail, not a send: it lands, may wake the session within its budget as any person's act does, and refills that budget (§4.10). Beside **Send**, which types into the pane and is the act of control (TD-052). One dialog shared with Reply; an `ask` takes the default bound. The composer's placeholder asks the shape §4.10 *Written to be read cold* asks of a session — *what it is about, what you decided, what they should do; the reasons after a blank line* (TD-127; not built — TD-138) |
...
not built — TD-136; the parts were proposed on 2026-09-18 and designed beside TD-079 as built, as
...
1. **Unread** from the moment it lands. It never ages out *while the record lives*: an unread
   ...
   not built — TD-136). An `ask`'s bound runs from when it was sent, and expiry does not wait
...
**Written to be read cold** (TD-127; Paul, 2026-09-24: *they look like they were written for
another agent to read … I do not need all the gritty details, just the basic context, and the
decision*; designed the same day, not built — TD-138). Anything a session sends to the person —
an `ask`, a `steer`, a `note`, a reply that will be filed as *answered for you* (§4.9b) — is
written for someone who was not in the conversation, and **its first paragraph is the whole of
what the person needs**: what it is about, what was decided or is being asked, and what, if
anything, they have to do — one to three plain sentences; then a blank line; then the reading,
the evidence, the sections measured against. A reply with a `--source` opens with its verdict
(*merged #517* / *findings on #517: two*). It is **a shape rule, not a second text**: no
summarising model sits in the path, since the sender knows what it meant and a second text is a
second thing to get wrong; the presets say the rule in the same words (`techlead.md`, `manager.md`,
`grinder.md`, the designer's brief), and the Inbox draws the shape (§4.5a *Inbox row: details*).
**Markdown, a fixed subset**: paragraphs, emphasis, inline code and fenced code, lists, and links —
rendered by agentorc's own renderer of that subset and nothing else (§4.5a): raw HTML is text,
an image is text, a script is text, and a link is drawn as a link only for an `https://` address,
opening in a new tab, never as a button; a heading, and anything else outside the subset, is
text — §4.5's rule stands, *nothing on the page is built from text a session wrote except as
text*, and a link is text with an address. The same shape is asked of
a person writing to a session, in the composer's own placeholder (§4.5a *Message*).
