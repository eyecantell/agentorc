...
```
  ...
  A profile absent here has no line on any window. On a node the file is the node's own, as its `hosts.yml` is (§4.4a — policies that stop run on the node, from its replica; the page's edit reaches the home's file, and a node's is edited there until the replica carries settings).
...
- **Usage gate** (per profile; **redesigned 2026-09-22, TD-100, the shape decided by Paul the same
  ...
  *about 30% of each session and 10% of the weekly*), so the setting is a **reserve** per window
  ...
  is clamped to 0–100. With 10 a day the weekly line is 30% on the reset day, 60% with four days
  ...
  tool's own 100% shows `limited`, as before. **A pause is a send, not a kill** (tdgrind's PAUSE
  flag, §8): the host agent submits the record's `pause_prompt` once — *pause: finish the step in
  ...
  `gated: {profile, label, pct, line, since, next}`, `next` being when the line next moves or the
  window resets, whichever is sooner. The wording travels on the record as `wrapup_prompt` does,
  and for the same reason. While gated: the doorbell does not ring the session, a controller's
  `send` is refused with the gate as the reason (a person's own, from Focus, is not — taking over is
  §9 invariant 5's way out), and the card's slot reads ***paused · usage** — grind week 71% ≥ 70%,
  line moves Thu 07:00* as *what explains a stop* (§4.5 *The card's anatomy*, row 5 (a)): a mark,
  not a state — the session still reads `idle`. **Resume** is the record's `resume_prompt` —
  *resume: carry on from where you paused* — sent once when every window of the profile is under
  ...
  pauses whole and needs no declaration: the mark says why it is quiet, and TD-099's *stopped for
  the usage window* becomes a paused team, not an ended one. The run window's wrap-up-then-kill is
