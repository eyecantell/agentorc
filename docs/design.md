...
- Board write-back (the RPC is built, TD-069 step 3; the Inbox's board rows press it since #472).
  **Snooze** (edit the `Due:` date, or add one) and **Done** (`- [ ]` → `- [x]`) on a dev-cadence
  ...
  2026-09-24; not built — TD-142) is the third edit: the person's words appended to the item's own
  line as ` — Paul, <date>: <text>` (`agentorc: reply on <item> (session <name>)`) — an item is one
