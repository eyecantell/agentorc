...
6. **Inbox** (`/inbox`, 2026-09-19; TD-069 — the one place to work from): one centred column (*Layout*, below — it was full width until 2026-09-20), a **view over
   ...
   **Layout (2026-09-20, TD-082 — design round 2, after Paul's day with the page at full width;
   mockup `Inbox.dc.html`, not yet built).** Three things were wrong and one rule fixes each.
   *The page is one centred column*, 1100 px at most: a queue reads in order, top to bottom, and
   ...
   fold is its heading**: the section stays a native disclosure, its heading line the toggle (the
   mockup's *fold ▴* is that line's own marker, not a second control), and an FYI entry newer than
   this browser last saw is marked *new* — the same comparison that opens the section by itself
...
- **Resume carries mail forward.** Resuming a conversation creates a new record and closes the
  ...
  (2026-09-20, TD-081 — the one-press Resume, §4.5a) has no second record to move mail *to*: the
  name check answers `supersede`, and the new record replaces the old one **under the same id**.
  As built, that path (`_take_name`) starts the new record with an empty inbox, outbox, tallies and
  `sends` — right for a *new* conversation that takes over a dead one's name, and wrong for a
  resume — while `_supersede` looks only for *another* record, in `exited`, holding the tool's
  session id, so it finds nothing. The rule: **when a create both supersedes a holder by name and
