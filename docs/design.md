...
- Board write-back (the RPC is built, TD-069 step 3; the Inbox's board rows press it since #472).
  **Snooze** (edit the `Due:` date, or add one) and **Done** (`- [ ]` → `- [x]`) on a dev-cadence
  ...
  checkout never sits dirty and the history is auditable. One **add** (designed, TD-069 step 4; not
  built — TD-140): **Put on the board** on an FYI row (§4.5a) writes one new line at the top of the
  ...
  action, due}`** and **`board_reply {board, line, text, reply, refs}`** (`refs` the references the
...
2. **Focus**: embedded terminal (full conversation; **on an `interactive` session the keyboard
   ...
   **Whose session it is** decides two things below. A session is the person's **own** when it
   ...
   1. **The identity line** — what this session is: ← Org, the **name alone** (host and repo are
      ...
      set and still the control that edits it; *out of work*; *restart wanted*; *paused ·
   ...
   4. **The side panel** — every card a fold, remembered per browser as a team's fold is, in the
      ...
      — the checklist with a small Close beside its heading, **open on an own session** and
      **folded on a team member**, whose note says its team closes it; and **Session**, last and
      **folded**: profile, adapter id, tmux, directory, started, last output, mode, the stops
      control when no stop time is set, run log, and the two chips the header carried until
      this review, **grants** and **controllers** — both read at a session's start and rarely
...
| Focus header | **Close session** | the acts line's next act, outlined, on the person's **own** session — `interactive`, or no team badge — when Ready to close passes on an `idle` or `exited` record (§4.5 *The Focus screen's anatomy*, TD-156): the same kill + reap → `closed` as the card's. The identity line carries ***ready to close ✓*** in the card's words at the same moment. An unattended team member never has it here: its team's Wind down or Start closes it, and its **Close** is the *more ▾* entry. The side panel's Ready to close keeps a small Close beside its heading, the press beside the reason |
