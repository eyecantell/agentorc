...
| **Paperclip** (MIT, https://paperclip.ing, [ADR](decisions/2026-09-24-paperclip.md)) | an "AI company" control plane: org chart, goals, tickets; each agent a finite heartbeat run per wake, resumed by session id, over twelve runtimes; Postgres, one server, sandboxed execution workspaces; dollar budgets with hard stops | continuation reconstructed from records (and its looping postmortem, for §6's ceilings); wake admission as a pure proceed/coalesce/defer function; budget policy as one generic row (TD-128); decisions — options whose effects the server runs on a press (here a press sends text to the sender, §4.10; only the staleness check carries over); two-layer notices with evidence on demand (TD-127) | no live session to take over; permission prompts skipped by default; work lives in its database, not the repo; cents, not usage windows |
...
- **Repo facts** (§4.5 screens 1 and 9, TD-170): for every checkout the registry lists, at the
  ...
  *other*. **Opened and closed in a window** come from the ledger file's git history in the same
  ...
  dev-cadence reads those fields (its readers are the board's and the sweep's; screen 6's *never
  a second parser* is about the board); TD-159 weighs whether it belongs there. **Phases** are
  derived on the view from the members' `progress` and the entries (§4.5 screen 1): a claim on a
  design-first entry is *design*, a claim with a PR *review*, without one *grind*, and *add* is
  reserved for a hunter's filing (TD-160 and later). **The doing log** (§4.8): beside the record's
...
- Board write-back (the RPC is built, TD-069 step 3; the Inbox's board rows press it since #472).
  **Snooze** (edit the `Due:` date, or add one) and **Done** (`- [ ]` → `- [x]`) on a dev-cadence
  `user_attention.md` item are one-line edits the host agent makes and commits with a fixed message
  naming the item's head and the session that raised it (`agentorc: snooze <item> to <date> (session
  <name>)`, `agentorc: done <item> (session <name>)`; `n/a` when the item names none), so the main
  ...
  the line would be read by nothing; on the line, still due, it reaches the next session that reads
  the board — through the Inbox row and `--json` at once, and through the SessionStart hook once the
  reader prints a line's ` — Paul, <date>:` tails whole after the head it clips at 200 characters
  (`MAX_ITEM_CHARS`), which is asked of dev-cadence with the fields below. Either place needs the
  reader's help to reach the hook; the line keeps the item's identity, its line number and every
  other row's, which an inserted line would shift. The calls are **`board_edit {board, line, text,
  action, due}`** (the add: `action: add` with `text`, `due` and `entry`, the Inbox row it comes
...
1. **Org** (home): a **card grid**, grouped by team when any live session carries a team badge
   ...
   **The Org, team-first (TD-170; Paul, 2026-09-26, four rounds on a design canvas).** Once a
   ...
   six-row card above stays for the sessions a person drives alone (*No team*). Rendered:
   `docs/mockups/reviews/2026-09-26-org-team-first-design.png`; the artboard is
   `docs/mockups/OrgTeamFirst.dc.html`.

   - **The rollup**, one row of four facets under the title, every number a link (§4.5a *Org
     rollup*): **Agents (n)** — every state's count as a plain coloured pill, *needs you (1)*,
     *working (5)*, in the grid's urgency order, the count in parentheses after the word as
     everywhere; **TDs in motion (n)** — a stacked bar by phase (below) with a legend; **PRs in
     motion (n)** — the window picker (day · week · month) and two sized blocks, *opened* and
     *closed* in the window, with the open count and how many wait on review under them; **Needs
     you (n)** — *answer needed* (a permission or question on a member) and *in the Inbox* (the
     Inbox's Needs you count) with the overdue count. No feed at this level: the person goes to a
     team for that. Sums over every live team.
   - **A team's card**: the header (name, `host / repo`, the session count, *answered for you*,
     Wind down / Stop now) — no state chips, the member cards say it — then **three facets**,
     then the members. **Repo** (§4.5a *team card: Repo facet*): the repo's name, a link to its
     page; **Technical debt (n open)** with its own selector *open · day · week · month* — on
     *open* two stacked bars, the open entries by priority (High, Medium, Low, one hue darkened
     by priority, the counts inside) and by kind (pickable, design-first, for you, other, the
     numbers inside and a legend under), on a period two sized blocks, entries *opened* and
     *closed* in it; **Pull requests (n open)** with the window picker and the two blocks; under
     them the oldest PR's age and how many wait on review, and when the readings were taken.
     **TDs in motion (n)** (§4.5a *team card: TDs in motion*): every reference a member holds,
     one row each — its **phase**, the reference, the entry's title, the member, its PR when it
     has one; a person's own session on the team marked with the person glyph. **Answer needed
     (n)** / **Doing** (§4.5a *team card: Answer needed / Doing*): one facet with a toggle. While
     a member waits on a permission or a question, the facet opens on **Answer needed**, tinted,
     with the member, what it asks, Allow / Deny and the *why?* box, or the question's text;
     otherwise it opens on **Doing**, the team's live feed of `ao doing` calls (time, doer,
     words), newest first, the last fifty kept (§4.8 *the doing log*). The toggle is a person's
     to flip either way. A **phase** is derived from the record, never declared: **add** — a
     hunter is filing the entry (the role is TD-160's and later; the phase is named now so the
     bar does not change shape when it comes); **design** — a designer holds a design-first
     entry; **grind** — a claim with no PR; **review** — a claim with an open PR.
   - **A member's compact card** (§4.5a *card: compact*): the name and, on a person's own session,
     the person glyph; the state pill at the right, **word and colour only** — the state glyphs
     (§4.5a *state icon*) leave the compact card and every count pill, since the summary above
     says what a page of glyphs was for; one line of its own — the role and its claim with the
     PR (*Grinder · TD-301 → #811*), or its ending (*out of work*), or the seat's word (*last
     came 2h ago*), or *Manager · round 41*; and the foot, the same rule as the full card's
     (the next act outlined first, the rest plain, *more* at the right). A `needs-you` member
     keeps the amber ring and its foot leads with Focus; the answer is given in the facet.
   - **No team** keeps the full card, and **a team with nothing live**, or concluded, is one row:
     name, place, the counts, *stopped* / *concluded <t> ago*, Start, the fold and Forget all.
...
2. **Focus**: embedded terminal (full conversation; **on an `interactive` session the keyboard
   ...
   **The Focus screen's anatomy (TD-156).** The card's rule (*The card's anatomy*, above)
   applied to Focus: the same things in the same places whatever the session's state, the next
   act as the one outlined button, and what is rarely pressed behind **more ▾**. The header is
   **two lines**, an identity line and an acts line — the split TD-085 named as the other answer
   and left as a §4.5a decision; Paul's review of a designer's Focus decided it (2026-09-25): one
   wrapping row of twenty things broke over three lines at 1440 with the buttons split between
   two of them, and the one press the end of a session needs, Close, sat at the bottom of the
   side panel.
   **Whose session it is** decides two things below. A session is the person's **own** when it
   is `interactive` or carries no team badge: theirs to close, one at a time (§2, *only the
   person closes it*). An unattended team member runs itself: its team's Wind down or Start
   closes it (§4.9a), a person presumes it behaves, and Focus does not press them to close it —
   nor is there a team-level close for the finished ones (Paul, 2026-09-25: *I would expect the
   team members to run themselves*).
   1. **The identity line** — what this session is: ← Org, the **name alone** (host and repo are
      the Org's and the Session card's), the tool's title only when it differs from the name (the
      card's rule), the state pill with the pending's Allow / Deny, its *why?* box and its text
      while the session needs the person, then the marks — ***ready to close ✓*** on an own
      session, the card's caption in the card's words, the moment the checklist passes on an
      `idle` or `exited` session; *suspended*; the *stops* note, drawn only when a stop time is
      set and still pressable to edit it; *out of work*; *restart wanted*; *paused ·
      usage*. Nothing on this line is a button but the answer to a permission and the stops
      note, and nothing on it is a sentence: what the session says it is doing is the Working
      card.
   2. **The acts line** — what a person may do: the **next act first, outlined**, by state —
      **Close session** on an own session when the checklist passes (the same kill and reap as
      ...
      the checklist passes) and **Kill**, last and red, where the card keeps it. Kill is never
      the only stop in sight: an own session the checklist passes offers Close where Kill stood,
      and Kill is one menu away for the case where the worktree must be kept.
   3. **The terminal**, the banners above it and the composer below it: unchanged.
   4. **The side panel** — every card a fold, remembered per browser as a team's fold is, in the
      order a person reads them: **Working** (open, drawn only while the session says what it is
      doing — the `doing` line's words with their age, its own and never acted on; what it has
      finished is Reports); **Git** (open); **Reports** and **Inbox** (open, drawn only when
      non-empty, the count in the heading); **Members** on a manager (open); **Ready to close**
      — the checklist, **open on an own session** with a small Close beside its heading, and
      **folded on a team member** with no button, whose note says its team closes it; and **Session**, last and
      **folded**: profile, adapter id, tmux, directory, started, last output, mode, the stops
      control (set or not — the identity line repeats it as a note while one is set), run log,
      and the two chips the header carried until this review, **grants** and **controllers** — both read at a session's start and rarely
      pressed after (TD-036). Its summary line is the profile, so the fold says what it holds.

   **What Wrap up promises.** A person's Wrap up is only the prompt (§4.5a); nothing closes the
   session but the person (§2). So the button says what happens next — its hover reads *asks it
   to finish, push and report, the same prompt the policy sends; you close it when Ready to
   close passes* — and the press toasts the same, so nobody waits for a close that is theirs.
...
6. **Inbox** (`/inbox`; TD-069 — the one place to work from): one centred column (*Layout*,
   ...
   *Snooze*), behind a small *n snoozed — show* that lists it with **Unsnooze**: a snooze is never
   ...
   Focus on the session that needs the person, while that record still exists. **The rail**
   (below; TD-129) narrows every section — a state and a message carry their session's `team`
   badge; a board item carries its repo, which `org.yml`'s projects map to teams — and its
   **find** box matches every word typed against a row's whole visible text. The Org keeps
   every needs-you mark it has: the Inbox is a second way to the same things, not a replacement.
   Nothing on the page is built from text a session wrote except as text — a rendered message is
   text, from a closed markdown subset and the UI's own renderer, and a link to an outside host is
   the one pressable thing it may make, drawn as one (§4.10, TD-127): a suggested answer
   ...
   disagree; the whole message is on the page with no scroll box — its first paragraph drawn, the rest one press away under *details* (TD-127). The poll is **a person's read of the
   ...
   destination is known before the press) whose **Snooze ▾**, **Done** and **Reply** are §4.4's
   ...
   **Find (TD-129).** The rail's box. Every word typed must match, in any order, as a substring of
   the row's whole visible text, which the server lowercases once into `data-find` on every row kind
   — a mail row's sender, team, kind, `about`, PR and text; a state row's name, pill, title, `doing`
   and its explanation; a board row's repo and text; a trail row's name, kind, `how` and text; the
   identity-alarm row's host and alarm words — and a word that is a number also matches `#` before
   it (*517* finds *#517*). A match inside a folded section (FYI, the snoozed list) unfolds it for
   the duration of the find and folds it back when the box empties, unless the person had it open;
   the count beside the box reads *n of all*. **The find is a fourth group with one pick, AND'd with
   the rail's** (Paul, 2026-09-24): *guardians* pressed and *jeff* typed shows the guardians' rows
   that carry *jeff* — *jeffrey* among them, a word matching as a substring — and the rail's counts
   follow the find as they follow every other pick. Nothing is written: it is the browser's filter
   over the rows it has, and the poll re-applies it. `/` focuses it and `Esc` leaves it (TD-124).
   What it replaces: one exact substring (`includes`) over sender, text and `about`, under which two
   words in the other order, `#` before a number or a comma after one found nothing, and a match in
   a fold stayed folded.

   **The message page (TD-129; mockup `InboxMessage.dc.html`).** `/inbox/<id>`: one mail entry,
...
9. **Repo** (`/repo/<name>`; designed 2026-09-25 with Paul, reshaped 2026-09-26 on the canvas —
   ...
   **The page opens with the team's three facets**, exactly as the team card draws them (screen
   ...
   same selectors, so nothing is learned twice. Under them, at the Org's width, the lists behind
   the numbers, a section a heading with its count and its *i* mark (§4.5a *Inbox: section
   ...
   3. **Waiting on you** — the repo's board items that are due: the Inbox's own board rows with
      ...
      sees both repos' rows there, the Inbox knowing teams and not repos).
   4. **Doing** — the team's `ao doing` calls in the window, newest first, with one row of
      **filter chips** above it: *all (n)* and one per agent that appears in the window with its
      count, *orc-1 (12)*; a chip shows that agent's calls alone, *all* the lot (§4.5a *Repo page:
      doing filters*).
   The page re-reads on the Inbox's poll and takes the `repos` and `doing` events as the card
   does. Narrow (below 720 px): one column, the facets stacked. **Not a dashboard**: no history
   beyond the window, and the lists are the things themselves. **A repo two teams share** is on
   the first team's card, in definition order (`repo_teams`, screen 6), and its page names every
   servicing team. Where the numbers come from is §4.4 *Repo facts*; the CLI's read is `ao repo`
   (§4.7).
...
| Focus | **Wrap up** | sends the wrap-up prompt (same one the policy uses). Its hover and the press's toast say what happens next — *asks it to finish, push and report, the same prompt the policy sends; you close it when Ready to close passes* — since a person's Wrap up closes nothing (TD-156) |
| Focus header | **Close session** | the acts line's next act, outlined, on the person's **own** session — `interactive`, or no team badge — when Ready to close passes on an `idle` or `exited` record (§4.5 *The Focus screen's anatomy*, TD-156): the same kill + reap → `closed` as the card's. The identity line carries ***ready to close ✓*** in the card's words at the same moment. An unattended team member never has it here: its team's Wind down or Start closes it, and its **Close** is the *more ▾* entry. On an own session the side panel's Ready to close keeps a small Close beside its heading, the press beside the reason; a member's checklist has no button, its Close being the *more ▾* entry |
| Focus header | **more ▾** | Pop out · Copy · Paste (each naming its keys) · Hand back / Switch to unattended (when the mode toggle is not the next act) · Copy tmux command · **Close** (enabled once Ready to close passes; on a team member the only place it is offered) · **Kill** — what is rarely pressed, behind one menu as on the card (TD-156) |
...
| Repo page | **links** | display: a PR number to its GitHub page; an author, member or doer name to its Focus; an entry id to its row in the Technical debt list; the *Waiting on you* heading to the Inbox filtered as the facet's *in the Inbox* is; the team name to the Org filtered to that team; *← Org* back |
...
| Inbox row: FYI | **Put on the board** | the one door from mail to a board, drawn on FYI rows only — a `note`, a closed question, a trail row — and never on an open `ask` or `steer`, whose *later* is **Snooze** (a session waiting on a question is in the queue, not on a file it never reads). It opens a small form: the board (the sender's repo's `docs/user_attention.md`, or a pick from the host's registered repos when the sender has none — a `system` note), the text prefilled from the entry and editable (a board line is *what's needed* in the person's words), and **Due** — +1 day · +1 week · a date, as Snooze offers; a host whose repos carry no board says so in the form and offers no Confirm. Confirm writes one line in the board's own format — `- [ ] <today> (session <sender's name> on <host>, or n/a) — <text>. Context: <about>. Due: <date>.` — at the top of the open items through the board write-back's one **add** (§4.4: committed with a fixed message naming the entry's id, never pushed), then deletes the entry as Dismiss does; a refusal (the checkout off its default branch or dirty, the entry already gone) is drawn in the form, which stays open, and a line committed whose entry would not dismiss says so rather than failing; the new board item is a counted row from the moment it is due, as any is. The rule sessions follow — *anything with a date on it belongs on the board* — given to the person as one press |
...
question about the caller's clock — and gated as an acting call (`set_start`, beside `set_stop`).
`ao status -v` prints *starts 20:00* by the *stops* formatter.
...
- **The doing log** (TD-170; Paul, 2026-09-26: *a live feed that shows the `ao doing` calls for
  ...
  **Doing** section draw. Nothing reads the log but a page and `ao repo`; no wake, no policy.
  ...
  (*round 41: reading four members, two claims to re-check*), which is also the line its card and
  the TDs in motion facet show under the manager.
...
wraps them up — is the `manager`; the technical go-between is the `techlead`; the team's
`designer` — an `unattended` member, one per repo team, whose lane is the ledger's `design-first`
entries: it writes this document and the entries the grinders pick, merges nothing on a held path,
and decides for itself how much of a design the person sees (TD-120): an **obvious** design is
designed, fact-checked and merged, and the person gets an uncounted FYI `note` with the PR; a
**defensible default the person might care about** is a `steer` with the default and a bound never
shorter than a night, the PR opened at once and merged at the bound unless answered; **taste,
...
- **Log TD** files the alarm where work is picked up (TD-077 b). The host agent does not write a
  ...
  none`, `mail.owed` and *Ready to close* all read. `identity_log` was the mark's only setter until
  TD-126 (2026-09-24): a board reply mailed to a lease holder is the second (§4.4 *Board
  write-back*, §4.5a *Inbox board row → Reply*), and owes the same outcome. The list is then cleared and the trail says *logged by you →
...
**When it is read: the sentence the sender sees (TD-158; designed 2026-09-25, not built — TD-168).**
The composer looks the same whoever it is addressed to, and what happens next differs by the
addressee's state: Paul mailed a note to the exited techlead and waited for it to come, and
nothing on the screen said the seat comes on a question. So every message carries, before it is
sent, **one sentence saying when it will be read**, computed in one place —
`sessionorc.mail.read_when(record, kind, now)`, a pure function of the addressee's record and the
kind — and drawn in the composer (§4.5a **Message**), returned by `msg` per addressee for `ao msg`
(§4.7), never stored. The sentence is the first of these that applies, in the doorbell's own order
(*A pending stop beats mail*, below), so it can never promise a ring the doorbell would not give:

| the addressee | the sentence |
|---|---|
...
an `ask` adds its bound (*an ask takes the default bound of n h*). A `reply` reads as a `note` does.
...
section tells sessions not to do.
...
clock runs, pause stops the clock, and both on one row invite the wrong press. A person's **Drop** on a claim from Focus (§4.5a *Reports*, TD-143) is told the same way — *your claim on TD-127 was dropped by the person from Focus — the lease is gone; claim again if you still hold the work* — so a worker learns its lease is gone before it pushes into a reference another session may have taken.
...
**How a message to a person is written (TD-127, 2026-09-24).** Anything a session sends to the
person — an `ask`, a `steer`, a `note`, a reply the home files as *answered for you* — is read cold,
by someone who did not do the work, so it has one shape: **the first paragraph is the whole of what
the person needs** — what it is about, what was decided or is being asked, and what, if anything,
they must do — in one to three plain sentences; then a blank line; then the reading — the evidence,
the sections it was measured against, the alternatives — for the record and for the asker. A
`--source` reply's first line is its verdict (*merged #517 — one gap left, not a blocker*). It is a
rule about one text, not a second text: Paul's first thought was a *details* option holding the
original under a humanised summary, and it was turned down because a summary field drifts from the
message it summarises, a session reading the same mail through `ao inbox` would read one or the
other, and a model put in the path to rewrite it is a second thing to get wrong — the sender knows
what it meant. The presets say the rule in these words — `techlead`, `manager`, `grinder`, the
designer's brief — and the person is asked the same thing the other way: the Message and Reply
dialog's placeholder (§4.5a **Message**) reads *first what you want, then why*. **The home warns, never refuses.** `ao msg` to
the person (`person`, or a `--source` reply, which is filed to the person) whose first paragraph
runs past `FIRST_PARA_WORDS` (60), or whose text has no blank line and runs past `FOLD_CHARS` (300),
prints one line — *the person reads the first paragraph: n words* — and sends: mail is never lost to
a style rule. **Markdown, a closed subset**: paragraphs, emphasis, inline code, code blocks, bullet
and numbered lists, and links — no headings, images, tables or raw HTML — rendered by the UI's **own
renderer** (`agentorc.ui.render`, a hundred lines that emit escaped HTML and nothing else), not a
vendored library behind a sanitiser, so that *no raw HTML reaches a page* is a property of the code
rather than of a configuration; everything outside the subset is drawn as the characters typed. **A
link is the one pressable thing text can make**, and §4.5's rule (*nothing on the page is built
from text a session wrote except as text*, TD-071 item 8) is kept by drawing one only when it is
absolute, `http` or `https`, and not the UI's own origin; it opens in a new tab and carries its host
in small print after its text, so link text can never pass for one of the page's controls; any other
link is its characters. `ao inbox` prints the source as written. **The row draws the first paragraph
and folds the rest** under *details* (§4.5a *Inbox row: details*), and the message page opens it
(§4.5 screen 6 *The message page*).
...
§4.5a *Inbox board row → Reply*), it lands as a `note` from the person marked `handed` (§4.8a),
so the holder owes `--outcome done|blocked|dropped --for <id>` as for any work the person handed
it, the row waits under *Waiting on them*, and the board line itself stays due until whoever
carries the instruction out closes it as any board line is closed.
...
- **A bounded exchange, counted by thread.** Messages in one thread are counted, and past the
  ...
  **`bound_hit`** mark on the thread that both cards and every participant's `ao` replies show — so
  ...
  commits to a board on nobody's behalf (board write-back, §4.4: an edit, or one add, at a
...
- **Start time** (`start_at`; TD-026, decided by Paul 2026-09-25 — *keep start_at and a
  ...
  running*) — in the state **`scheduled`**: no pane, no run log yet, the directory's agent slot
  held from this moment (§9 invariant 2: a second session there is refused as if it were live),
  and `supervised` as any supervised create. At the instant the home's tick creates the session
  from the launch record (`restarts: [{why: start}]`, under `RESTART_CEILING` as any replay is, a
  failed create counted and shown), and the record goes on as any session's; a `scheduled` record
  ...
  instant in the past (say `now`), and a start time on a live session (`ao at` is for `scheduled`
  records; a live session already started). It is the per-session form of *Schedule*, below: a
