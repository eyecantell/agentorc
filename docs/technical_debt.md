...
| TD-033 | `tests/test_cli.py::test_explain_file_and_session` flaked once in a full run | Low | Open |
...
**Priority:** Medium
**Added:** 2026-09-10
**Status:** Open — design landed (§4.8); step (1) merged 2026-09-10 (caller on every RPC, the `orchestrate` gate, `capabilities` with `ao new --grant` / `ao grant` / `ao revoke`); steps (2)–(3) merged 2026-09-11 (the two channels, `ao progress` / `ao finding`, `ao new --lane`, the skill's declare rule; the derived source on the tick); steps (4)–(5) pending
...
## TD-033: `tests/test_cli.py::test_explain_file_and_session` flaked once in a full run

**Priority:** Low
**Added:** 2026-09-11
**Status:** Open — seen once (2026-09-11, `pdm run test` during TD-029); three full runs and the test alone passed after it, so it is a race, not a break
**Location:** `tests/test_cli.py::test_explain_file_and_session`, `src/sessionorc/agent.py` (`rpc_explain`, the screen verdict)

**Why:** One full-suite run failed the test on an assertion while the same test passed on its own and in the next three full runs. The suite classifies a live pane's screen, so the likely race is the usual one in this file (TD-025's shape): the assertion runs before the pane has painted what the rule matches, or a tick's `scraped` verdict lands between the two reads. Left unfixed rather than papered over with a longer sleep, because which assertion failed was not captured — the next occurrence should record the output before anything is changed.

**Fix:** on the next occurrence, capture the failing assertion and its `ao explain --json`, then make the test wait for the screen it asserts on (as `wait_state` does for state) rather than reading once. TD-025 is the same class of flake in the same file; if both point at the same read-once pattern, fix them together.
