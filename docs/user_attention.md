...
- [ ] 2026-09-10 (session 38caa51c `tdgrind-ao-1` run 2 on kmaster) — **Merged, live check pending:** `agentorc-agent serve` now stops by cancelling the serve task (TD-024) — after the next `systemctl --user restart agentorc-agent`, `journalctl --user -u agentorc-agent` should show no `Task was destroyed` ERROR line and the old socket file gone. Context: TD-024 (archive). Due: 2026-09-13.
