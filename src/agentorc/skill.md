...
- **Answer an `ask` or a `steer`**: `ao msg --reply-to <id> "…"` goes back to its sender (answered from what is written down: add `--source "<file and section>"`, and the person is told), or `--pick <n>` sends one of its suggested answers by the number `ao inbox` prints (from 1); offer your own with `--answer "<line>"`, up to four, and read a reply's `answer` index rather than its text. Kinds: `note`, `ask` (`--bound S`), `steer` (`--default "<the line you will go with>"`), `reply`, `conflict` (`--cites` the `sends` ids `ao status -v` prints).
