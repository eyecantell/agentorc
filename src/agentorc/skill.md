...
- `ao msg <id>… "…" [--about TD-NNN]` reaches only your controllers, your members, your team, or a controller of a session you control; a refusal names the rule; the reply ends with when each addressee will read it (`read_when`) — an exited one on its resume, a seat on a question. **Never broadcast.**
